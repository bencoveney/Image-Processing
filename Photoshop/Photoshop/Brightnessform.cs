...
namespace Photoshop
{
    public partial class Brightnessform : Form
    {
        ...
        private void Main(string OpenFile, string SaveFile, int NewBrightness)
        {
            if (OpenFile != null && SaveFile != null)
            {
                ...
                for (int y = 0; y < HeightInPixels; y++)
                {
                    ...
                    for (int x = 0; x < WidthInBytes; x = x + BytesPerPixel)
                    {
                        ...
                        Pixels[CurrentLine + x] = (byte)(OldBlue + NewBrightness);
                        if (OldBlue + NewBrightness > 255) Pixels[CurrentLine + x] = 255;
                        if(OldBlue+NewBrightness < 0) Pixels[CurrentLine + x] = 1;

                        Pixels[CurrentLine + x + 1] = (byte)(OldGreen + NewBrightness);
                        if (OldGreen + NewBrightness > 255) Pixels[CurrentLine + x + 1] = 255;
                        if (OldGreen + NewBrightness < 0) Pixels[CurrentLine + x + 1] = 1;


                        Pixels[CurrentLine + x + 2] = (byte)(OldRed + NewBrightness);
                        if (OldRed + NewBrightness > 255) Pixels[CurrentLine + x + 2] = 255;
                        if (OldRed + NewBrightness < 0) Pixels[CurrentLine + x + 2] = 1;
                    }
                }
        ...
        {
            int NewBrightness = BrightnesshScrollBar.Value;
            string OpenFile = OpenFiletextBox.Text;
            string SaveFile = SaveFiletextBox.Text;
            Main(OpenFile, SaveFile, NewBrightness);
        }
    }
}
